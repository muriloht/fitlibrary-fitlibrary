...
public class TestGlobalScope {
	...
	public void getBecomesTimeout() {
		context.checking(new Expectations() {{
			oneOf(runtimeContext).getTimeout(GlobalScope.BECOMES_TIMEOUT, 1000); will(returnValue(1000));
		}});
	...
	public void setBecomesTimeout() {
		context.checking(new Expectations() {{
			oneOf(runtimeContext).putTimeout(GlobalScope.BECOMES_TIMEOUT, 5000);
		}});
	...
	public void theStopWatchNotStarted() {
		context.checking(new Expectations() {{
			oneOf(dynamicVariables).get(GlobalScope.STOP_WATCH); will(returnValue(null));
		}});
	...
	public void startAndstopStopWatch() {
		context.checking(new Expectations() {{
			oneOf(dynamicVariables).put(with(GlobalScope.STOP_WATCH),with(aNonNull(StopWatch.class)));
