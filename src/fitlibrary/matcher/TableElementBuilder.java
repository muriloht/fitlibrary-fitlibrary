...
public abstract class TableElementBuilder<From extends TableElement, Builder extends TableElementBuilder, To extends TableElement> {
	...
	public From mock(final Mockery context, String path, int index) {
		...
		context.checking(new Expectations() {{
			...
			allowing(from).iterator(); will(returnIterator(listOfMockElements));
			allowing(from).getType(); will(returnValue(type.getSimpleName()));
			allowing(from).getLeader(); will(returnValue(leader));
			allowing(from).getTrailer(); will(returnValue(trailer));
			allowing(from).getTagLine(); will(returnValue(tagLine));
			for (int i = 0; i < elementBuilders.size(); i++) {
				final int ii = i;
				allowing(from).at(i); will(returnValue(listOfMockElements.get(ii)));
			}
		}});
		if (listOfMockElements.isEmpty())
			...
			context.checking(new Expectations() {{
				allowing(from).last(); will(returnValue(listOfMockElements.get(elementBuilders.size()-1)));
			}});
		return from;
	}
	...
	protected void with(TableElementBuilder<From, Builder, To> copy, 
			Builder... els) {
