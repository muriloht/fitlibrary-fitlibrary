...
public class PositionedTargetWasFound implements PositionedTarget {
	...
	public PositionedTargetWasFound(Evaluator evaluator, String[] cells,
			TypedObject typedObject, Method method, int innerFrom, int innerUpTo, boolean sequencing, LookupMethodTarget lookupTarget) {
	...
	public String ambiguityErrorMessage() {
		return "Special "+specialName+"("+args(specialTarget)+") + "+
			innerName+"("+args(innerTarget)+") (in "+specialTarget.getOwningClass().getName()+
			" + "+innerTarget.getOwningClass().getName()+")";
	}
	@Override
