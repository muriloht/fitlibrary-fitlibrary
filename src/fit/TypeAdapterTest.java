...
public class TypeAdapterTest extends TestCase {
	...
	public void testTypeAdapter() throws Exception {
		...
		adapter = TypeAdapter.on(f, f.getClass()
				.getMethod("pi", new Class[] {}));
		assertEquals(3.14159, ((Double) adapter.invoke()).doubleValue(),
				0.00001);
		...
		assertTrue(adapter.equals(new int[] { 1, 2, 3 }, f.sampleArray));

		adapter = TypeAdapter.on(f, f.getClass().getField("sampleDate"));
		Date date = new GregorianCalendar(49 + 1900, 4, 26).getTime();
		String format = DateFormat.getDateInstance(DateFormat.SHORT).format(
				date);
	...
	private void assertBooleanTypeAdapterParses(String booleanString,
			boolean assertedValue) throws Exception {
	...
	public void testParseDelegateObjectMethod() throws Exception {
		...
		TypeAdapter.registerParseDelegate(Date.class, new SimpleDateFormat(
				dateFormat));
	...
	public void testParsedelegateClassShouldHavePublicStaticNonVoidParseMethodWithStringParam()
			throws Exception {
		TypeAdapter.registerParseDelegate(Class.class,
				PublicStaticParseMethod.class);
	}
	...
	public void testShouldThrowNoSuchMethodExceptionIfGivenParseDelgateClassDoesNotHaveStaticParseMethod()
		...
		try {
			TypeAdapter.registerParseDelegate(Class.class,
					PublicNonStaticParseMethod.class);
		} catch (RuntimeException e) {
			assertEquals("Parse delegate class "
	...
	public void testShouldThrowNoSuchMethodExceptionIfGivenParseDelgateClassHasParseMethodReturningVoid()
		...
		try {
			TypeAdapter.registerParseDelegate(Class.class,
					PublicStaticVoidParseMethod.class);
		} catch (RuntimeException e) {
			assertEquals("Parse delegate class "
	...
	public void testShouldThrowNoSuchMethodExceptionIfGivenParseDelgateClassDoesNotHaveParseMethodWithStringParam()
		...
		try {
			TypeAdapter.registerParseDelegate(Class.class,
					PublicStaticParseMethodWithoutStringParam.class);
		} catch (RuntimeException e) {
			assertEquals("Parse delegate class "
