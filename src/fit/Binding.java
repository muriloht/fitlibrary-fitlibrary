...
public abstract class Binding {
	...
	private static TypeAdapter makeAdapter(Fixture fixture, String name)
			throws Throwable {
	...
	private static TypeAdapter makeAdapterForMethod(String name,
			Fixture fixture, Matcher matcher) {
	...
	private static TypeAdapter makeAdapterForRegexMethod(String name,
			Fixture fixture, Matcher matcher) {
		Method method = getMethod(name, fixture, matcher);
	...
	private static Method getMethod(String name, Fixture fixture,
			Matcher matcher) {
		Method method = null;
		if (GracefulNamer.isGracefulName(name)) {
			String simpleName = GracefulNamer.disgrace(name).toLowerCase();
			...
			try {
				...
				method = fixture.getTargetClass().getMethod(methodName,
						new Class[] {});
	...
	public static class SaveBinding extends Binding {
		...
		public void doCell(Fixture fixture, Parse cell) {
			try {
				...
				String symbolValue = valueObj == null ? "null" : valueObj
						.toString();
